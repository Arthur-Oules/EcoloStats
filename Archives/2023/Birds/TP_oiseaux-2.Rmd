...
```{r, include=F}
data_oiseaux <- read.csv("/home/marie/Documents/Enseignement/EnseignementNew/Enseignement/CoursL3/UEEcoloAnalyseData/Rendus2023/Rendus_groupe/Oiseau/data_oiseaux.csv")
...
```{r, include=FALSE}
data_reponse_env <- subset(data_oiseaux, select=c('ENSOLEILLEMENT', 'PUBLIC', 'HEURE', 'VENT_MOY', 'VENT_MAX', 'TEMPERATURE'))
data_reponse_env_sup <- subset(data_oiseaux, select=c('INDICE_REP','ENSOLEILLEMENT', 'PUBLIC', 'HEURE', 'VENT_MOY', 'VENT_MAX', 'TEMPERATURE'))

acp_env <- dudi.pca(data_reponse_env, scannf = FALSE, nf = 3)
acp_env_sup <- PCA(data_reponse_env_sup, scale.unit=T,ncp=3,quanti.sup=1)
```

```{r}
data_oiseaux <- cbind(data_oiseaux, acp_env$li[,2])
colnames(data_oiseaux) <- c(colnames(data_oiseaux[1:24]), ('INDICE_ENV'))
```
...
```{r, include=FALSE}
plot(data_oiseaux$INDICE_REP ~ data_oiseaux$INDICE_ENV, ylab="Indice de réponse",xlab="Indice env", las=2)
lines(lowess(data_oiseaux$INDICE_ENV,data_oiseaux$INDICE_REP))
```
...
```{r}
chants <- read.csv("/home/marie/Documents/Enseignement/EnseignementNew/Enseignement/CoursL3/UEEcoloAnalyseData/Rendus2023/Rendus_groupe/Oiseau/Chants.csv")
chants <- chants[-8]
...
```{r}
par(mfrow=c(2,3))
hist(chants$PROPORTION_SIF, main='Histogramme de la prop sif', xlab='Proportion sif')
boxplot(PROPORTION_SIF~ID_MALE,data=chants)
boxplot(PROPORTION_SIF~DIFFUSION,data=chants, las=2, xlab="")
boxplot(PROPORTION_SIF~NUMERO,data=chants)
plot(PROPORTION_SIF~DUREE,data=chants)
lines(lowess(chants$PROPORTION_SIF~chants$DUREE))

par(mfrow=c(2,3))
hist(chants$DUREE, main='Histogramme de la durée des chants', xlab='Durée')
boxplot(DUREE~ID_MALE,data=chants)
boxplot(DUREE~DIFFUSION,data=chants, las=2, xlab="")
...
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE, title="Effet du mâle sur la réponse", axis.labels=c("MALE 6","MALE 5","MALE 3","MALE 2","MALE 1"))
```
...
sjPlot::plot_model(anova_rep, show.values=TRUE, show.p=TRUE, title='Effet de la diffusion sur la réponse', axis.labels=c("FTNsif", "FTNgaz", "FTNcomp", "FJ", "FG"))
```
...
```{r}
par(mfrow=c(1,2))
hist(chants$PROPORTION_SIF, main='Histogramme de la prop sif', xlab="Proportion sifflée")
boxplot(chants$PROPORTION_SIF ~ chants$DIFFUSION, main='Prop sif en fonction de diffusion', ylab='Prop sif', xlab='', las=3)
```
...
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE, title="Effet du jour sur la réponse", axis.labels=c("JOUR 4","JOUR 3","JOUR 2"))
```

Sur les 4 jours, il ne semble pas y avoir d'habituation, regardons l'indice de session.

```{r}
plot(data_oiseaux$INDICE_REP ~ data_oiseaux$SESSION_DIFF, main= 'Indice de réponse', ylab='Indice de réponse', xlab='Session')
...
summary(anova)
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE, title="Effet de la météo sur la réponse", axis.labels=c("Indice météo"))
```
...
```{r, include=FALSE}
anova <- lm(data_oiseaux$INDICE_REP ~ data_oiseaux$INDICE_ENV)
summary(anova)
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE, title="Effet de l'env sur la réponse", axis.labels=c("Indice env"))
```
...
data_oiseaux$ID_MALE <- as.factor(data_oiseaux$ID_MALE)
data_oiseaux$ID_MALE <- relevel(data_oiseaux$ID_MALE, "4")
anova <- lm(data_oiseaux$STROPHES_APRES ~ data_oiseaux$ID_MALE) 
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE, title="Effet du mâle sur la réponse", axis.labels=c("MALE 6","MALE 5","MALE 3","MALE 2","MALE 1"))
```
...
```{r, include=F}
anova_nlme=lme(STROPHES_APRES ~DIFFUSION, random=~1 | as.factor(ID_MALE), data=data_oiseaux)
print(contrast(anova_nlme, contr_group, ftn_group, cnames=paste0(c("TN", "FG", "FJ", "FTNgaz", "FTNsif"), " vs FTNcomp")))
```
...
data_oiseaux$JOUR <- as.factor(data_oiseaux$JOUR)
data_oiseaux$JOUR <- relevel(data_oiseaux$JOUR, "1")
anova <- lm(data_oiseaux$STROPHES_APRES ~ data_oiseaux$JOUR)
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE)

plot(data_oiseaux$STROPHES_APRES ~ data_oiseaux$SESSION_DIFF, main= 'Nombre de strophes post-diffusion', ylab='Nombre de strophes post-diffusion', xlab='Session')
lines(lowess(data_oiseaux$STROPHES_APRES ~ data_oiseaux$SESSION_DIFF))
anova <- lm(data_oiseaux$STROPHES_APRES ~ data_oiseaux$SESSION_DIFF)
```

### Les conditions météos influencent-elles la réponse ?

```{r, include=TRUE}
anova <- lm(data_oiseaux$STROPHES_APRES ~ data_oiseaux$INDICE_METEO)
sjPlot::plot_model(anova, show.values=TRUE, show.p=TRUE)
```

Les effets des différentes variables explicatives sur le nombre de strophes après diffusion sont les mêmes que sur l'indice de réponse. On en déduit que nos résultats sont plutôt robustes, et nos interprértations sont les mêmes.
